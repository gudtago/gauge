...
func ConnectToRunner(killChannel chan bool, debug bool, reporter io.Writer) (runner.Runner, error) {
	manifest, err := manifest.ProjectManifest()
	if err != nil {
		return nil, err
	}

	runner, connErr := runner.Start(manifest, reporter, killChannel, debug)
	if connErr != nil {
		return nil, connErr
	}
